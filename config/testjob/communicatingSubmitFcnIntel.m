...
additionalSubmitArgs = cellstr(additionalSubmitArgs); temp = cellstr('-N testjob -l walltime=00:30:00 -j oe -m cmiller@osc.edu 8gb'); C = cell(1, 2); C = [ additionalSubmitArgs temp ]; additionalSubmitArgs = strjoin(C);dctSchedulerMessage(4, '%s: Requesting %d nodes with %d processors per node', currFilename, ...

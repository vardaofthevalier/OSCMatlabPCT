...
runMpiexec() {
    INTEL_MPIEXEC=/usr/local/intel/impi/4.0.3.008/intel64/bin/mpiexec.hydra
    # As a debug stage: echo the command line...
    echo ${INTEL_MPIEXEC} -print-rank-map -n ${MDCE_TOTAL_TASKS} -hostfile ${PBS_NODEFILE} \
        -envlist MDCE_DECODE_FUNCTION,MDCE_STORAGE_LOCATION,MDCE_STORAGE_CONSTRUCTOR,MDCE_JOB_LOCATION,MDCE_DEBUG,MDCE_LICENSE_NUMBER,MLM_WEB_LICENSE,MLM_WEB_USER_CRED,MLM_WEB_ID \
        ${MDCE_MATLAB_EXE} ${MDCE_MATLAB_ARGS}
    eco $MDCE_LICENSE_NUMBER
    
    # ...and then execute it
    eval ${INTEL_MPIEXEC} -print-rank-map -n ${MDCE_TOTAL_TASKS} -hostfile ${PBS_NODEFILE} \
        -envlist MDCE_DECODE_FUNCTION,MDCE_STORAGE_LOCATION,MDCE_STORAGE_CONSTRUCTOR,MDCE_JOB_LOCATION,MDCE_DEBUG,MDCE_LICENSE_NUMBER,MLM_WEB_LICENSE,MLM_WEB_USER_CRED,MLM_WEB_ID \
        ${MDCE_MATLAB_EXE} ${MDCE_MATLAB_ARGS}
...
MAIN() {
    ...
    cat ${PPBS_NODEFILE}
    # Make symlink to mpiLibConf because the file must exist before any matlab process starts
    # Or, copy the file
    #cp -v ${HOME}/matlab/mpi/mpiLibConf.m ${HOME}/matlab/mpiLibConf.m
    #${MDCE_STORAGE_LOCATION}/${MDCE_JOB_LOCATION}/mpiLibConf.m
    export MATLABPATH=/usr/local/MATLAB/OSC/distcomp/mpi
    chooseMachineArg
    runMpiexec
    exit ${MPIEXEC_CODE}
}
